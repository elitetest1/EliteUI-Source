.class public Lcom/samsung/android/app/SemExecutableWhitelist;
.super Ljava/lang/Object;
.source "SemExecutableWhitelist.java"


# static fields
.field private static final blacklist DEBUG:Z

.field private static final blacklist SIGNATURE_MAX:I = 0x3

.field private static final blacklist SIGNATURE_SAMSUNG_APPLICATION:I = 0x2

.field private static final blacklist SIGNATURE_SAMSUNG_PLATFORM:I = 0x1

.field private static final blacklist SIGNATURE_SAMSUNG_SHARED:I = 0x4

.field private static final blacklist TAG:Ljava/lang/String; = "SemExecutableWhitelist"

.field private static blacklist sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;


# instance fields
.field private blacklist mSignaturesMap:Landroid/util/SparseArray;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroid/util/SparseArray<",
            "Landroid/content/pm/Signature;",
            ">;"
        }
    .end annotation
.end field

.field private blacklist mWhiteList:Ljava/util/HashMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/HashMap<",
            "Ljava/lang/String;",
            "Ljava/lang/Integer;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method static constructor blacklist <clinit>()V
    .locals 1

    invoke-static {}, Landroid/os/Debug;->semIsProductDev()Z

    move-result v0

    sput-boolean v0, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    const/4 v0, 0x0

    sput-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    return-void
.end method

.method public constructor blacklist <init>()V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    new-instance v0, Landroid/util/SparseArray;

    invoke-direct {v0}, Landroid/util/SparseArray;-><init>()V

    iput-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v0, Ljava/util/HashMap;

    invoke-direct {v0}, Ljava/util/HashMap;-><init>()V

    iput-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mWhiteList:Ljava/util/HashMap;

    return-void
.end method

.method private blacklist SemExecutableWhitelist()V
    .locals 0

    invoke-direct {p0}, Lcom/samsung/android/app/SemExecutableWhitelist;->setupSignaturesMap()V

    invoke-direct {p0}, Lcom/samsung/android/app/SemExecutableWhitelist;->setupWhiteList()V

    return-void
.end method

.method public static blacklist getInstance()Lcom/samsung/android/app/SemExecutableWhitelist;
    .locals 1

    sget-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    if-nez v0, :cond_0

    new-instance v0, Lcom/samsung/android/app/SemExecutableWhitelist;

    invoke-direct {v0}, Lcom/samsung/android/app/SemExecutableWhitelist;-><init>()V

    sput-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    :cond_0
    sget-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    return-object v0
.end method

.method private blacklist getSignatures(I)Ljava/util/ArrayList;
    .locals 4
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(I)",
            "Ljava/util/ArrayList<",
            "Landroid/content/pm/Signature;",
            ">;"
        }
    .end annotation

    new-instance v0, Ljava/util/ArrayList;

    invoke-direct {v0}, Ljava/util/ArrayList;-><init>()V

    const/4 v1, 0x0

    :goto_0
    const/4 v2, 0x3

    if-ge v1, v2, :cond_1

    const/4 v2, 0x1

    shl-int/2addr v2, v1

    and-int v3, p1, v2

    if-eqz v3, :cond_0

    iget-object v3, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    invoke-virtual {v3, v2}, Landroid/util/SparseArray;->get(I)Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Landroid/content/pm/Signature;

    invoke-virtual {v0, v2}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    :cond_0
    add-int/lit8 v1, v1, 0x1

    goto :goto_0

    :cond_1
    return-object v0
.end method

.method private blacklist hasSamsungKey([Landroid/content/pm/Signature;)Z
    .locals 1

    const/4 v0, 0x7

    invoke-direct {p0, p1, v0}, Lcom/samsung/android/app/SemExecutableWhitelist;->isMatchedSignature([Landroid/content/pm/Signature;I)Z

    move-result p0

    return p0
.end method

.method private blacklist isMatchedSignature([Landroid/content/pm/Signature;I)Z
    .locals 5

    invoke-direct {p0, p2}, Lcom/samsung/android/app/SemExecutableWhitelist;->getSignatures(I)Ljava/util/ArrayList;

    move-result-object p0

    const/4 p2, 0x0

    if-eqz p1, :cond_2

    if-eqz p0, :cond_2

    array-length v0, p1

    move v1, p2

    :goto_0
    if-ge v1, v0, :cond_2

    aget-object v2, p1, v1

    invoke-virtual {p0}, Ljava/util/ArrayList;->iterator()Ljava/util/Iterator;

    move-result-object v3

    :cond_0
    invoke-interface {v3}, Ljava/util/Iterator;->hasNext()Z

    move-result v4

    if-eqz v4, :cond_1

    invoke-interface {v3}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v4

    check-cast v4, Landroid/content/pm/Signature;

    if-eqz v4, :cond_0

    invoke-virtual {v4, v2}, Landroid/content/pm/Signature;->equals(Ljava/lang/Object;)Z

    move-result v4

    if-eqz v4, :cond_0

    const/4 p0, 0x1

    return p0

    :cond_1
    add-int/lit8 v1, v1, 0x1

    goto :goto_0

    :cond_2
    return p2
.end method

.method private blacklist setupSignaturesMap()V
    .locals 3

    iget-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v1, Landroid/content/pm/Signature;

    const-string v2, ""

    invoke-direct {v1, v2}, Landroid/content/pm/Signature;-><init>(Ljava/lang/String;)V

    const/4 v2, 0x1

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseArray;->put(ILjava/lang/Object;)V

    iget-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v1, Landroid/content/pm/Signature;

    const-string v2, ""

    invoke-direct {v1, v2}, Landroid/content/pm/Signature;-><init>(Ljava/lang/String;)V

    const/4 v2, 0x2

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseArray;->put(ILjava/lang/Object;)V

    iget-object p0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v0, Landroid/content/pm/Signature;

    const-string v1, ""

    invoke-direct {v0, v1}, Landroid/content/pm/Signature;-><init>(Ljava/lang/String;)V

    const/4 v1, 0x4

    invoke-virtual {p0, v1, v0}, Landroid/util/SparseArray;->put(ILjava/lang/Object;)V

    return-void
.end method

.method private blacklist setupWhiteList()V
    .locals 0

    return-void
.end method


# virtual methods
.method public blacklist isAllowedToUseOrder(Landroid/content/Context;Ljava/lang/String;)Z
    .locals 4

    const-string v0, "SemExecutableWhitelist"

    const/4 v1, 0x0

    :try_start_0
    invoke-virtual {p1}, Landroid/content/Context;->getPackageManager()Landroid/content/pm/PackageManager;

    move-result-object p1

    const/16 v2, 0x40

    invoke-virtual {p1, p2, v2}, Landroid/content/pm/PackageManager;->getPackageInfo(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;

    move-result-object p1

    if-nez p1, :cond_1

    sget-boolean p0, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz p0, :cond_0

    const-string p0, "isAllowedToUse : pkgInfo is null"

    invoke-static {v0, p0}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I

    :cond_0
    return v1

    :cond_1
    iget-object v2, p1, Landroid/content/pm/PackageInfo;->applicationInfo:Landroid/content/pm/ApplicationInfo;

    iget v2, v2, Landroid/content/pm/ApplicationInfo;->flags:I

    and-int/lit16 v2, v2, 0x81

    const/4 v3, 0x1

    if-eqz v2, :cond_2

    return v3

    :cond_2
    iget-object p1, p1, Landroid/content/pm/PackageInfo;->signatures:[Landroid/content/pm/Signature;

    invoke-direct {p0, p1}, Lcom/samsung/android/app/SemExecutableWhitelist;->hasSamsungKey([Landroid/content/pm/Signature;)Z

    move-result v2

    if-eqz v2, :cond_3

    return v3

    :cond_3
    iget-object v2, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mWhiteList:Ljava/util/HashMap;

    invoke-virtual {v2, p2}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object p2

    check-cast p2, Ljava/lang/Integer;

    if-nez p2, :cond_5

    sget-boolean p0, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz p0, :cond_4

    const-string p0, "isAllowedToUse : executable manager white lists does not include this App"

    invoke-static {v0, p0}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I

    :cond_4
    return v1

    :cond_5
    invoke-virtual {p2}, Ljava/lang/Integer;->intValue()I

    move-result p2

    invoke-direct {p0, p1, p2}, Lcom/samsung/android/app/SemExecutableWhitelist;->isMatchedSignature([Landroid/content/pm/Signature;I)Z

    move-result p0

    if-nez p0, :cond_6

    sget-boolean p1, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz p1, :cond_6

    const-string p1, "isAllowedToUse : executable manager white lists don\'t match this App"

    invoke-static {v0, p1}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I
    :try_end_0
    .catch Landroid/content/pm/PackageManager$NameNotFoundException; {:try_start_0 .. :try_end_0} :catch_0

    :cond_6
    return p0

    :catch_0
    move-exception p0

    sget-boolean p1, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz p1, :cond_7

    new-instance p1, Ljava/lang/StringBuilder;

    const-string p2, "isNotAllowedToUse : "

    invoke-direct {p1, p2}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p1, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    invoke-virtual {p1}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-static {v0, p0}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I

    :cond_7
    return v1
.end method
