.class public Landroid/webkit/WebView$PrivateAccess;
.super Ljava/lang/Object;
.source "WebView.java"


# annotations
.annotation runtime Landroid/annotation/SystemApi;
.end annotation

.annotation system Ldalvik/annotation/EnclosingClass;
    value = Landroid/webkit/WebView;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x1
    name = "PrivateAccess"
.end annotation


# instance fields
.field final synthetic blacklist this$0:Landroid/webkit/WebView;


# direct methods
.method public constructor whitelist <init>(Landroid/webkit/WebView;)V
    .locals 0
    .annotation system Ldalvik/annotation/MethodParameters;
        accessFlags = {
            0x8010
        }
        names = {
            null
        }
    .end annotation

    iput-object p1, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method


# virtual methods
.method public whitelist awakenScrollBars(I)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$1400(Landroid/webkit/WebView;I)Z

    return-void
.end method

.method public whitelist awakenScrollBars(IZ)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2}, Landroid/webkit/WebView;->access$1500(Landroid/webkit/WebView;IZ)Z

    return-void
.end method

.method public whitelist getHorizontalScrollFactor()F
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0}, Landroid/webkit/WebView;->access$1700(Landroid/webkit/WebView;)F

    move-result p0

    return p0
.end method

.method public whitelist getHorizontalScrollbarHeight()I
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0}, Landroid/webkit/WebView;->access$1900(Landroid/webkit/WebView;)I

    move-result p0

    return p0
.end method

.method public whitelist getVerticalScrollFactor()F
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0}, Landroid/webkit/WebView;->access$1600(Landroid/webkit/WebView;)F

    move-result p0

    return p0
.end method

.method public whitelist onScrollChanged(IIII)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-virtual {p0, p1, p2, p3, p4}, Landroid/webkit/WebView;->onScrollChanged(IIII)V

    return-void
.end method

.method public whitelist overScrollBy(IIIIIIIIZ)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static/range {p0 .. p9}, Landroid/webkit/WebView;->access$1300(Landroid/webkit/WebView;IIIIIIIIZ)Z

    return-void
.end method

.method public whitelist setMeasuredDimension(II)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2}, Landroid/webkit/WebView;->access$1800(Landroid/webkit/WebView;II)V

    return-void
.end method

.method public whitelist setScrollXRaw(I)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$2102(Landroid/webkit/WebView;I)I

    return-void
.end method

.method public whitelist setScrollYRaw(I)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$2202(Landroid/webkit/WebView;I)I

    return-void
.end method

.method public whitelist super_computeScroll()V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0}, Landroid/webkit/WebView;->access$201(Landroid/webkit/WebView;)V

    return-void
.end method

.method public whitelist super_dispatchKeyEvent(Landroid/view/KeyEvent;)Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$701(Landroid/webkit/WebView;Landroid/view/KeyEvent;)Z

    move-result p0

    return p0
.end method

.method public whitelist super_getScrollBarStyle()I
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0}, Landroid/webkit/WebView;->access$001(Landroid/webkit/WebView;)I

    move-result p0

    return p0
.end method

.method public whitelist super_onApplyWindowInsets(Landroid/view/WindowInsets;)Landroid/view/WindowInsets;
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$1201(Landroid/webkit/WebView;Landroid/view/WindowInsets;)Landroid/view/WindowInsets;

    move-result-object p0

    return-object p0
.end method

.method public whitelist super_onDrawVerticalScrollBar(Landroid/graphics/Canvas;Landroid/graphics/drawable/Drawable;IIII)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static/range {p0 .. p6}, Landroid/webkit/WebView;->access$2001(Landroid/webkit/WebView;Landroid/graphics/Canvas;Landroid/graphics/drawable/Drawable;IIII)V

    return-void
.end method

.method public whitelist super_onGenericMotionEvent(Landroid/view/MotionEvent;)Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$801(Landroid/webkit/WebView;Landroid/view/MotionEvent;)Z

    move-result p0

    return p0
.end method

.method public whitelist super_onHoverEvent(Landroid/view/MotionEvent;)Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$301(Landroid/webkit/WebView;Landroid/view/MotionEvent;)Z

    move-result p0

    return p0
.end method

.method public whitelist super_performAccessibilityAction(ILandroid/os/Bundle;)Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2}, Landroid/webkit/WebView;->access$401(Landroid/webkit/WebView;ILandroid/os/Bundle;)Z

    move-result p0

    return p0
.end method

.method public whitelist super_performLongClick()Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0}, Landroid/webkit/WebView;->access$501(Landroid/webkit/WebView;)Z

    move-result p0

    return p0
.end method

.method public whitelist super_requestFocus(ILandroid/graphics/Rect;)Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2}, Landroid/webkit/WebView;->access$901(Landroid/webkit/WebView;ILandroid/graphics/Rect;)Z

    move-result p0

    return p0
.end method

.method public whitelist super_scrollTo(II)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2}, Landroid/webkit/WebView;->access$101(Landroid/webkit/WebView;II)V

    return-void
.end method

.method public whitelist super_setFrame(IIII)Z
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2, p3, p4}, Landroid/webkit/WebView;->access$601(Landroid/webkit/WebView;IIII)Z

    move-result p0

    return p0
.end method

.method public whitelist super_setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1}, Landroid/webkit/WebView;->access$1001(Landroid/webkit/WebView;Landroid/view/ViewGroup$LayoutParams;)V

    return-void
.end method

.method public whitelist super_startActivityForResult(Landroid/content/Intent;I)V
    .locals 0

    iget-object p0, p0, Landroid/webkit/WebView$PrivateAccess;->this$0:Landroid/webkit/WebView;

    invoke-static {p0, p1, p2}, Landroid/webkit/WebView;->access$1101(Landroid/webkit/WebView;Landroid/content/Intent;I)V

    return-void
.end method
